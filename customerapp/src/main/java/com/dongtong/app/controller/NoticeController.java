...
public class NoticeController {
    ...
    public ResultDO<Boolean> IsUnRead(
        ...
        return noticeAO.IsUnRead(ReceiveType.CUSTOMER.getValue());

    }

    @ApiOperation(value = "帖子评论人数", notes = "帖子评论人数")
    @RequestMapping(value = "/selectBusNoticeCount/{version:.+}", method = RequestMethod.POST)
    public ResultDO<Integer> selectBusNoticeCount(
            @ApiParam(name="version",value="版本号", required=true, defaultValue ="v1.0.0" )@PathVariable("version") String version){
        return noticeAO.selectBusNoticeCount();
